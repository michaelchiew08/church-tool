...
class PinyinConverterNotifier extends Notifier<String> {
  ...
  void convertText(String text) {
    ...
    for (final element in linesOfText) {
      final pinyinState = ref.read(pinyinToneNotifierProvider);
      if (pinyinState == PinyinFormatType.withToneMark) {
        state = '$state\n${titleCase(
      ...
      if (pinyinState == PinyinFormatType.withToneNumber) {
        state = '$state\n${titleCase(
          PinyinHelper.getPinyin(
            element,
      ...
      if (pinyinState == PinyinFormatType.withoutTone) {
        state = '$state\n${titleCase(
          PinyinHelper.getPinyin(
            element,
          ),
